...
parser.add_argument('--predictions', default='', type=str,
                    help='Full path to file with predictions')

parser.add_argument('--hdf5', default='', type=str, nargs="*",
                    help='Full path to hdf5 files that were use for ' +
                    'predictions separated by a space, or using a ' +
                    'wildcard ("*")')

parser.add_argument('--DIS', default=False, action='store_false',
                    help='Choose whether to make DIS cuts (W > 2 GeV)')
            ...
            for key in index_dict:
                for entry in range(len(index_dict[key])):
                    data_dict[key][i][entry] = \
                        currentline[index_dict[key][entry]]
...
def mult_plot(actuals, predictions=None, n_tracks=None,
              probabilities=None, suffix=''):
    if suffix == '0':
    ...
    if predictions is not None and probabilities is not None:
        yerr2 = np.zeros(6)
        probs = probabilities[np.arange(predictions.shape[0]), predictions]
        for i in range(6):
            iprobs = probs[predictions == i]
...
#                plt.legend()
#                plt.title(prefix + 'Predicted probability')
#                plt.ylabel('Events')
#                plt.xlabel('Probability')
#                plt.savefig('probability{}.pdf'.format(i))
#                plt.close()
...
#                plt.ylabel('Events')
#                plt.xlabel('Probability')
#                plt.savefig('all_probability{}.pdf'.format(i))
#                plt.close()



#            var = np.power(1 - iprobs, 2).sum()/iprobs.shape[0]
            yerr2[i] = iprobs.shape[0] * np.std(iprobs) #iprobs.shape[0] * np.sqrt(var)
#        expt_val = np.sum(probabilities, axis=0)
            print( np.var(iprobs),  np.std(iprobs), np.var(probabilities[:,i]), np.std(probabilities[:,i]))
        yerr2 = ns2 * np.std(probabilities, axis=0)

        yerr1 = np.sqrt(ns1)
        yerr2 += np.sqrt(ns2)
        ax1.errorbar(np.arange(6), ns1, yerr=yerr1,
            marker='.', linestyle='', color='red')
        ax1.errorbar(np.arange(6), ns2, yerr=yerr2, capsize=3,
    ...
    predictions_true = (
        predictions[actuals==0],
        predictions[actuals==1],
        predictions[actuals==2],
        predictions[actuals==3],
        predictions[actuals==4],
        predictions[actuals==5]
        )
    labels = (
        'Truth 0',
        'Truth 1',
    ...
    plt.ticklabel_format(axis='y', style='sci', scilimits=(0, 2))
    plt.legend()
    plt.title(prefix + 'Predicted multiplicity')
    plt.ylabel('Events')
    plt.xlabel('Hadron Multiplicity')
    plt.savefig('multperbin'+ suffix +'.pdf')
    plt.close()
...
def main(actuals, data_dict, W, Q2, n_tracks=None, DIS=None):
...
#----------------------------------cuts----------------------------------------
    ...
    probabilities = probabilities[Q2>DoLimQ2]
    if n_tracks is not None:
...
#-----------------------------------plots---------------------------------------
    ...
    if n_tracks is not None:
        ...
        mult_plot(actuals0, predictions0,
            probabilities=probabilities0, suffix='0')
        mult_plot(actuals1, predictions1,
            probabilities=probabilities1, suffix='1')
        mult_plot(actuals2, predictions2,
            probabilities=probabilities2, suffix='2')
    ...
    if n_tracks is not None:
        wdistplot(WT, bins, labelT, title="WTrackedDist.pdf",
            ylog = True, xlog=True)
...
if __name__ == '__main__':
    ...
    for hdf5 in hdf5_list:
        ...
        file_events = reader.openf()
        total_events += file_events
        actuals = np.append(actuals, reader.get_key(0, file_events, #hadro_data/n_hadmultmeas_100mev
                key='vtx_data/planecodes'))
        W = np.append(W, reader.get_key(0, file_events,
                key='gen_data/W')/1000)
        Q2 = np.append(Q2, reader.get_key(0, file_events,
                key='gen_data/Q2')/1000000)
        reader.closef()
        if total_events >= nevents:
          break
    actuals = actuals[:nevents]
    W = W[:nevents]
    Q2 = Q2[:nevents]
    data_dict['predictions'] = np.squeeze(data_dict['predictions'])

#    n_tracks[ n_tracks>5 ] = 5
    n_tracks = None

    main(actuals, data_dict, W, Q2, n_tracks, DIS=DIS)
